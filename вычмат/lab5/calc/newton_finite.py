...
def finite_differences(y):
    """Возвращает треугольную таблицу конечных разностей"""
    n = len(y)
    table = [y.copy()]
...
def interpolate_forward(x, x_sorted, y_sorted, h):
    """Интерполяция вперёд (для x в первой половине)"""
    n = len(x_sorted)
...
def interpolate_backward(x, x_sorted, y_sorted, h):
    """Интерполяция назад (для x во второй половине)"""
    n = len(x_sorted)
    table = finite_differences(y_sorted)
    ...
    for i in range(1, n):
        term *= (t + i - 1) / i
        # Берём последнюю разность i-го порядка
        if i < len(table[i]):
...
def interpolate(x, x_sorted, y_sorted, h=None):
    """Основная функция - вычисление в одной точке"""
...
def interpolate_line(x_grid, x_sorted, y_sorted):
    ...
    for x in x_grid:
        if x <= mid:
            y = interpolate_forward(x, x_sorted, y_sorted, h)
        else:
            y = interpolate_backward(x, x_sorted, y_sorted, h)
        result.append(y)
